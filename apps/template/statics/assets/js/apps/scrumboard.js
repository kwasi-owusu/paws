  ...
  function $_taskSortable() {
    ...
    $('[data-sortable="true"]').sortable({
      ...
      start: function (event, ui) {
        sourcePipeline = ui.item.parent().attr("id");
      },
      stop: function (event, ui) {
        let lead_ID = ui.item.attr("id");

        let pipeCors = $("#pipeCors").html();

        let targetPipeline = ui.item.parent().attr("id");

        $("#loader").show();

        $("#srcPipeline").html("Source pipeline: " + sourcePipeline);
        $("#leadID").html("Lead ID: " + lead_ID);
        $("#targetPipeline").html("Target pipeline: " + targetPipeline);
  ...
  function $_taskEdit() {
    $(".card .s-task-edit")
      ...
      .on("click", function (event) {
        ...
        var get_value_progress = $("#progress-range-counter").val(
          $_taskProgress
        );
        var get_value_progressHtml = $(".range-count-number").html(
          $_taskProgress
        );
        ...
        $('[data-btnfn="editTask"]')
          ...
          .on("click", function (event) {
            ...
            var $_taskProgressValue = $(".range-count-number").attr(
              "data-rangeCountNumber"
            );
