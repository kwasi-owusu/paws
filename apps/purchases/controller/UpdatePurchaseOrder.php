...
{
    static public function UpdateThisPurchaseOrder()
    {
        $error = false;
        ...
        if (isset($_SESSION['getPOCors']) && $_SESSION['getPOCors'] == $getToken) {
            ...
            $tbl = 'new_purch_oder';
            require_once('../../model/purchases/UpdateThisPurchaseOrder.php');
            if (!$error && $purchase_order_type == 1) {
                ...
                $grandTotal         = (float)$nhsAmount + $getFundAmount + $covidAmount + $vatAmount + $taxableAmount;


                if (UpdateThisPurchaseOrder::UpdateTaxablePO($sanitizeSupplCode, $sanitizeTerms, $sanitizeTpp, $sanitizeEts, $sanitizeEta,
                    ...
                    $sanitizeAmountDue, $sanitizePO_Details, $po_Key, $img_nm, $tbl, $folder, $purchase_order_type, $nhsAmount, $getFundAmount,
                    $covidAmount, $vatAmount, $totalBeforeVAT, $grandTotal, $po_ID, $lastUpdateBy)) {

                    echo "<span style='color: #ffffff'>Update Successful.</span>";
                } else {
                    echo "<span style='color: #ffffff'>Update Unsuccessful</span>";
                }
            }
            ...
            elseif (!$error && $purchase_order_type == 99) {
                ...
                if (UpdateThisPurchaseOrder::UpdateSpecialTaxablePO($sanitizeSupplCode, $sanitizeTerms, $sanitizeTpp, $sanitizeEts, $sanitizeEta,
                    $sanitizeFreight_type, $sanitizeTotal_approval, $sanitizePo_date, $sanitizePo_number, $sanitizeCurr, $sanitizeTotalAfterTax,
                    $sanitizeProductCode, $sanitizeItemName, $sanitizeQty, $sanitizePrice, $sanitizeTotal, $sanitizeFreight_amt, $sanitizeAmountPaid,
            ...
            elseif (!$error && $purchase_order_type == 2){
                ...
                if (UpdateThisPurchaseOrder::UpdateNonTaxablePurchaseOrderModel($sanitizeSupplCode, $sanitizeTerms, $sanitizeTpp, $sanitizeEts, $sanitizeEta,
                    $sanitizeFreight_type, $sanitizeTotal_approval, $sanitizePo_date, $sanitizePo_number, $sanitizeCurr, $sanitizeTotalAfterTax,
                    $sanitizeProductCode, $sanitizeItemName, $sanitizeQty, $sanitizePrice, $sanitizeTotal, $sanitizeFreight_amt, $sanitizeAmountPaid,
                    $sanitizeAmountDue, $sanitizePO_Details, $po_Key, $img_nm, $tbl, $folder, $purchase_order_type, $po_ID, $lastUpdateBy)) {
...
$callClass = new UpdatePurchaseOrder();
$callMethod = $callClass->UpdateThisPurchaseOrder();